...
public class AutoRunnerAnimator : MonoBehaviour
{
    ...
    public float speedWalk = 2f;        // NPC moves away slower when player is walking
    public float speedRun = -1f;        // NPC moves slightly toward player when player is running
    ...
    void Update()
    {
        ...
        if (mainPlayer != null)
        {
            ...
            if (horizontalInput == 0)
                ...
                moveDelta = speedWalk * Time.deltaTime;
            else
                moveDelta = speedRun * Time.deltaTime;
